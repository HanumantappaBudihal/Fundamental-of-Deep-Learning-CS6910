...
parser.add_argument("-rl", "--recurrent-layer", type=str, default="lstm", help="type of layer for encoder and decoder")
...
parser.add_argument("-as", "--attention-size", type=int, default=512, help="attention dimension for the decoder")
...
parser.add_argument("-o", "--optimizer", type=str, default="ranger", help="optimizer for training the model [should be one of `ranger` or `adamw`]")
...
def build_and_run_model_with_wandb(config=None):
    ...
    wandb.run.name = (
        f"ATTENTION"
        f"_layer_{config.recurrent_layer}"
    ...
    train_pred = model.predict([trainxe, trainxd[:, 0]], batch_size=config.batch_size)
    word_accuracy = compute_word_accuracy(train_pred.tolist(), trainy.tolist(), hin_tokens)

    # check validation-level accuracy as well
    # for prediction, pass the encoder input to the encoder and start-token to the decoder
    val_pred = model.predict([valxe, valxd[:, 0]], batch_size=config.batch_size)
    val_word_accuracy = compute_word_accuracy(val_pred.tolist(), valy.tolist(), hin_tokens)
...
def build_and_run_model_with_cmd_args():
    ...
    # create the attention-decoder with the required hyperparameters
    decoder = AttentionDecoder(input_dim=int(trainxd.max())+1, 
                               output_dim=int(trainy.max())+1, 
                               embed_dim=args.dec_embedding_size,
                               attn_dim=args.attention_size,
                               cell_hidden_dim=args.num_recurrent_units,
                               dropout=args.dropout,
                               k=args.num_dec_recurrent_layers, 
                               cell_type=args.recurrent_layer)

    # create the transliteration model with the created encoder and attention-decoder
    model = TransliterationModel(encoder=encoder, 
                                 decoder=decoder, 
                                 tgt_max_len=max_hin_len)
